...
```{r Obtención del dataframe, echo=TRUE}
datos <- tree_survival %>% 
...
```{r Visualizar datos perdidos, echo=TRUE}
datos_pr %>% 
  is.na() %>% 
  summary()
```
...
```{r  Representación gráfica de la distribución de fenoles, echo=FALSE, message=FALSE, warning=FALSE}
hipo1 %>% ggplot(aes(x = PHN_Imp, fill = Event )) +
  geom_histogram(color = "black") +
  labs(x= "Concentración de fenoles",y = "Número de individuos") +
  scale_x_continuous(
...
<!-- Los valores negativos de phenolics es porque son valores imputados y, por tanto, al azar -->
...
```{r  Análisis visual de concentración de fenoles, echo=FALSE, message=FALSE, warning=FALSE}
...
```{r Representación gráfica de la concentración de carbohidratos, echo=FALSE, message=FALSE, warning=FALSE}
hipo1 %>% ggplot(aes(x = NSC_Imp, fill = Event)) +
  geom_histogram(color = "black") + 
  labs(x="Concentración de carbohidratos solubles", y = "Número de individuos") +
  scale_x_continuous(
...
```{r Representación gráfica de la concentración de carbohidratos según supervivencia, echo=FALSE, message=FALSE, warning=FALSE}
hipo1 %>% ggplot(aes(y = NSC_Imp, x = Event, col = Event )) +
  geom_jitter(width = 0.2, show.legend = F, alpha = 0.6) +
  geom_boxplot(show.legend = F, width = 0.55) + 
  labs(x="Supervivencia", y="Concentración de carbohidratos")
...
```{r Representación gráfica de la concentración de fenoles según tratamiento, echo=FALSE, message=FALSE, warning=FALSE}
hipo2 %>% 
...
```{r Filtramos tratamientos, echo=TRUE, message=FALSE}
...
```{r Creamos variable de tratamiento "small", echo=TRUE}
smallPHN <- hipo2.1 %>% transmute (small = if_else(Microbe == "Small", PHN_Imp, NA)) %>% 
  drop_na()
```
```{r Creamos variable de tratamiento "large", echo=TRUE}
...
```{r Diferencias entre las variables, echo=FALSE, message=FALSE}
...
Al igual que antes al tener que hacer un **U-Mann-Whitney** (NSC no paramétricos) tenemos que hacer dos datasets, en este caso con los NSC.

```{r Creo variable 2.2.1, echo=TRUE}
smallNSC <- hipo2.1 %>%  transmute (small = if_else(Microbe == "Small", NSC_Imp, NA)) %>% 
  drop_na()
```

```{r Creo variable 2.2.2, echo=TRUE}
...
hipo2.1 %>% ggplot(aes(y = NSC_Imp, x = Microbe, col = Microbe )) +
  geom_jitter(width = 0.2, show.legend = F, alpha = 0.6) +
  geom_boxplot(show.legend = F, width = 0.55) + # Elimino la leyenda.
  labs(x="Tratamiento con microorganismos", y="Concentración de carbohidratos")
  theme(axis.title.x = element_text(hjust = 0.5),
        panel.border = element_rect(colour = "black", linewidth = 0.7, fill = NA),
        panel.background = element_blank()) 
```
...
```{r Relación AMF y PHN, echo=FALSE, message=FALSE}
 datos_pr %>% ggplot(aes(y = PHN_Imp, x = AMF_Imp )) +
  ...
  labs(x = "% Inclusiones micorrízicas", y = "Concentración de fenoles") +
...
```{r Relación AMF y NSC, echo=FALSE, message=FALSE}
 datos_pr %>% ggplot(aes(y = NSC_Imp, x = AMF_Imp )) +
  ...
  labs(x = "% Inclusiones micorrízicas", y = "Concentración de carbohidratos solubles") +
  geom_smooth(method = "lm")
